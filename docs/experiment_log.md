...
### 1.12 需求 4.2 对比基线（改进ICP vs 传统点到点ICP vs FGR，2026-09-23）

**需求文档 4.2 要求**「对比基线为传统点到点 ICP 和 FGR」。早期任务四的 FGR 数据
在非需求基线参数下（voxel=0.03/FPFH=0.40），且未做「无粗配准纯点到点ICP」。
本节约 15 分钟在**需求基线参数（voxel=0.02/FPFH=0.25/RANSAC=100k）**下补跑
组 B（纯点到点ICP，单位阵初始化）与组 C（FGR→改进ICP），各 3 次重复；
组 A（RANSAC→改进ICP）直接复用 §1.11 需求基线。脚本：
`scripts/compare_registration_baselines.py`；产物：
`results/registration/sweep_real/baselines_compare/{per_pair_results.csv,sweep_summary.json}`。

| 方法 | 中位成功率 | min–max | iv5 | iv10 | iv30 | 平均旋转 | 平均平移 | 平均耗时 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 组A：FPFH+RANSAC → 改进ICP（需求基线） | 25% | 15–25% | 4/7 | **0/7** | 1/6 | 8.60° | 0.379m | 6.90s |
| 组B：传统点到点ICP（无粗配准，单位阵初始化） | **35%** | 35%（确定性） | 3/7 | **4/7** | 0/6 | 5.04° | 0.110m | 0.80s |
| 组C：FGR → 改进ICP | 25% | 20–25% | 3/7 | 1/7 | 0/6 | 1.85° | 0.144m | 10.36s |

**反直觉但重要的结论**：

1. **在需求基线参数下，无粗配准的纯点到点ICP（35%）反而高于 RANSAC→改进ICP
   （25%）**——原因不是改进ICP 不如传统ICP，而是**原始参数的 RANSAC 粗配准
   在 iv10 上几乎全部给出错误初始化**（需求基线 iv10=0/7，纯ICP iv10=4/7）；
   近距离帧对（iv5/iv10，位姿差仅数度/数厘米）本就在点到点ICP 的收敛域内，
   单位阵初始化即可直接收敛，粗配准在此区间是**负资产**；
2. **粗配准的真正价值区间是远距离（iv30）**：纯ICP iv30=0/6（初始太远、局部极小
   不可逃逸），RANSAC→改进ICP iv30=1/6（唯一在 iv30 成功的方法）；但 iv30
   低重叠仍是结构性难题（见 §1.9/§1.10）；
3. **FGR→改进ICP 在本数据上无优势**（25%，且耗时 10.4s/对），与任务四结论一致；
4. **改进ICP 流水线的优势需要正确的粗配准初始化才能体现**：全调优组合
   （0.03/0.40/500k，§1.8/§1.11）中位 50%（iv5=5/7、iv10=3/7、iv30=2/6）
   **全面超过**纯点到点ICP 35%、FGR 25% 和需求基线 25%——调优后的粗配准
   在 iv10 从 0/7 修复到 3/7、iv30 从 1/6 到 2/6；
5. **对报告的意义**：需求基线 iv10=0/7 的根因是 RANSAC 粗配准参数欠调，而非
   改进ICP 本身——精配准质量已由成功子集精度证明（旋转中位 0.54°、平移
   0.029m，§3.5）；对比表最终口径建议采用「全调优 50% vs 纯点到点ICP 35% vs
   FGR 25%」，并注明近距离区间粗配准的双刃剑效应。
