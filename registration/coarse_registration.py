...
def coarse_registration(source, target, voxel_size=DEFAULT_VOXEL_SIZE,
    ...
    Args:
        ...
        mutual_filter: 是否启用对应点互滤波（True=只保留双向一致对应，
            默认启用；调参实验可关闭对比）。
    Returns:
    ...
    logger.info("粗配准参数: voxel=%.3f, fpfh_r=%.2f, iter=%d, thr=%.4f, mutual_filter=%s",
    ...
    logger.info("RANSAC 结果: fitness=%.4f, inlier_rmse=%.4f, n_corr=%d",
                info["fitness"], info["inlier_rmse"], info["n_correspondences"])
...
def main():
    ...
    parser.add_argument("--mutual_filter", type=lambda s: s.lower() in ("1", "true", "yes"),
                        default=True, help="对应点互滤波（0/1），默认 1")
